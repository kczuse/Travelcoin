...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Travelcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Travelcoin Signed Message:\n";
...
{
    // Travelcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 9 * COIN;
    if(nHeight = 1) // no block reward within the first 3 days
        nSubsidy = 2480000;
    if(nHeight > 1 && nHeight < 43201) 
        nSubsidy = 35;
    if(nHeight > 43200 && nHeight < 86401) 
        nSubsidy = 18;
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 120; 
static const int64 nTargetSpacing = 1 * 60; // Travelcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Travelcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Travelcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Travelcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1387210267, nBits=1e0ffff0, nNonce=2084514601, vtx=1)
        //   CTransaction(hash=97ddfbbae6, ver=1, vin.size=1, vout.size=1, nLockTime=0)
        //     CTxIn(COutPoint(0000000000, -1), coinbase 04ffff001d0104404e592054696d65732030352f4f63742f32303131205374657665204a6f62732c204170706c65e280997320566973696f6e6172792c2044696573206174203536)
        //     CTxOut(nValue=9.00000000, scriptPubKey=040184710fa689ad5023690c80f3a4)
        //   vMerkleTree: 97ddfbbae6

        // Genesis block
        const char* pszTimestamp = "NY Times December 16, 2013";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 9 * COIN;
        ...
        block.nVersion = 1;
        block.nTime    = 1387210267;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084514601;

        if (fTestNet)
        {
            block.nTime    = 1387210000;
            block.nNonce   = 180221584;
        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Travelcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TravelcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TravelcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TravelcoinMiner(CWallet *pwallet)
{
    printf("TravelcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Travelcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TravelcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TravelcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TravelcoinMiner, pwallet));
}
